...
void main() {
  ...
  group('PdfService Handwriting Generation Tests', () {
    test(
      'generateHandwriting does not hang and generates pages successfully',
      () async {
    ...
    test(
      'generateHandwriting in column mode does not hang and generates pages successfully',
      () async {
        final globalConfig = GlobalConfig(title: 'Test Title');
        final config = HandwritingConfig();
    ...
    test(
      'generateHandwriting supports all Zaner-Bloser grade level presets',
      () async {
        ...
        for (var grade in GradeLevel.values) {
          ...
          if (grade != GradeLevel.custom) {
            expect(
              config.effectiveWritingHeightPt,
              equals(grade.defaultWritingHeightPt),
            );
            expect(
              config.effectiveMidlineFromBaselinePt,
              equals(grade.defaultWritingHeightPt * 0.5),
            );
            expect(
              config.effectiveDescenderBufferPt,
              equals(grade.defaultWritingHeightPt * 0.5),
            );
          }
        }
    ...
    test(
      'generateHandwriting generates valid PDF in monochrome ink-saver mode',
      () async {
    ...
    test(
      'generateHandwriting supports all 4 HandwritingLineStyle choices',
      () async {
  ...
  group('PdfService Counting Generation Tests', () {
    test(
      'generateCounting in countAndWrite mode generates pages successfully',
      () async {
        final globalConfig = GlobalConfig(title: 'Test Title');
        ...
        final pdfBytes = await PdfService.generateCounting(
          globalConfig,
          config,
        );
        expect(pdfBytes, isNotEmpty);
    ...
    test(
      'generateCounting in drawToMatch mode generates pages successfully',
      () async {
        final globalConfig = GlobalConfig(title: 'Test Title');
        final config = CountingConfig(
